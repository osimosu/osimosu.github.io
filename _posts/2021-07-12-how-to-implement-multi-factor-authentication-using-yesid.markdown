--- 
layout: "post"
title:  "How to Implement Multi-Factor Authentication Using YesID"
date:   "2021-07-12 13:30:00 -0500"
categories: openid, oauth, ciba, identity, mfa, passwordless
---
This post describes how to implement Multi-factor authentication using [YesID](https://yesidentity.com){:target="_blank"
...
Below is a simplified diagram of how YesID enables multi-factor Authentication.
...
1. A user initiates a login using their credentials typically username and password. In your backend, you initiate an
   ...
   YesID's [Authorize Endpoint](https://datatracker.ietf.org/doc/html/rfc6749#section-3.1){:target="_blank"} using a
...
3. The device/app is notified of an Authentication request. YesID can notify the device on your behalf, or you can
...
Finally, let's implement Multi-factor authentication using YesID :)
...
Create a Client in the Dashboard to obtain credentials to authenticate with YesID's API. You can
...
When a user logs in to their **trusted** device for the first time, the device is registered with YesID's backend using
...
expiration date in the response body to the browser. In parallel, if configured, YesID notifies your device/app of a
...
While the request is pending (not yet approved by the user), YesID would return a json body with
...
is used in the demo app. Your endpoint should proxy this request securely to YesID's `Authentication Requests API`. Like
the `Devices API`, it is protected and expects a JWT token signed using the client's private key.

Depending on your security requirements, YesID supports approving Authentication requests on the device using biometric
